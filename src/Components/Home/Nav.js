...
const Nav = () => {
  ...
  useEffect(() => {
    if (user && user.email) {
      fetch(`http://localhost:8000/favorite/user/${user.email}`)
        .then((res) => res.json())
  ...
  useEffect(() => {
    if (user && user.email) {
      fetch(`http://localhost:8000/cart/user/${user.email}`)
        .then((res) => res.json())
  ...
  const fetchUserRole = async (email) => {
    try {
      const adminResponse = await fetch(`http://localhost:8000/admin/${email}`);
      const adminData = await adminResponse.json();
      setIsAdmin(adminData.admin);

      const vendorResponse = await fetch(
        `http://localhost:8000/vendor/${email}`
      );
